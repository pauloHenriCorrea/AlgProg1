...
def register(route=""):
    if route == "student":
        ...
        valid = False
        while not valid:
            name_dicipline = input("INFORME O NOME DA DISCIPLINA: ").upper()
            if name_dicipline and not name_dicipline.isnumeric():
    ...
    if route == "student":
        ...
        while not CPF_valid:
            ...
            if len(CPF) == 11:
                ...
                CPFs = add_CPFs_in_vetor(
                    lines
                )  # Vetor de CPFs que já foram cadastrados
        ...
        while not stop:
            name_dicipline = input(
                "INFORME O NOME DA DISCIPLINA QUE DESEJA EDITAR: "
            ).upper()
            exist = check_CPF_or_dicipline_exists(lines, name_dicipline)
            if exist:
                stop = True
    ...
    if route == "student":
        ...
        while not CPF_valid:
            ...
            if len(CPF) == 11:
                ...
                CPFs = add_CPFs_in_vetor(lines)
                exist_CPF = check_CPF_or_dicipline_exists(CPFs, CPF_formated, "remove")
        ...
        while not dicipline_valid:
            name_dicipline = input(
                "INFORME O NOME DA DISCIPLINA QUE DESEJA REMOVER"
            ).upper()
            diciplines = to_take_lines("data/diciplines.csv")
            exist = check_CPF_or_dicipline_exists(diciplines, name_dicipline, "remove")

            if exist[0]:
                i = 0
                fp = open("data/diciplines.csv")
